...
def test_build_charuco_info_lines_reports_actual_numbers():
    ...
    board it is."""
    lines = charuco.build_charuco_info_lines(
        squares_x=11, squares_y=8, square_size_mm=20.0, marker_size_mm=15.0
    )

    joined = " ".join(lines)
    assert "11 x 8" in joined
    assert "20.0 mm" in joined
    assert "15.0 mm" in joined
...
def test_write_pdf_letter_landscape_writes_a_file(tmp_path):
    ...
    charuco.write_pdf_letter_landscape(
        ...
        board_h_mm=50.0,
        margin_in=0.5,
        info_lines=charuco.build_charuco_info_lines(4, 3, 20.0, 15.0),
    )
